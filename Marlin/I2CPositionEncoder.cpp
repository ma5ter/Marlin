...
void I2CPositionEncoder::update()
{
	if (!initialised || !homed || !active) {
		return;    //check encoder is set up and active
	}
...
int32_t I2CPositionEncoder::get_raw_count()
{
	...
	encoderCount.val = (encoderCount.bval[2] & B00100000) ? (encoderCount.val | 0xFFC00000) :
					   (encoderCount.val & 0x003FFFFF);
	...
	if (I2CPE_idx == 0xFF) {
		LOOP_XYZE(i) {
			if (!I2CPE_anyaxis || parser.seen(axis_codes[i])) {
				const uint8_t idx = idx_from_axis(AxisEnum(i));
				if ((int8_t)idx >= 0) {
					calibrate_steps_mm(idx, iterations);
				}
			}
		}
	}
...
void I2CPositionEncodersMgr::M864()
{
	...
	if (!I2CPE_addr) {
		I2CPE_addr = I2CPE_PRESET_ADDR_X;
	}
