...
/**
    ...
    M0   - Unconditional stop - Wait for user to press a button on the LCD (Only if LCDKIND_CHARACTER is enabled)
    M1   -> M0
...
bool filament_sensor =
	false;                                 // M405 turns on filament sensor control. M406 turns it off.
float filament_width_nominal = DEFAULT_NOMINAL_FILAMENT_DIA,  // Nominal filament width. Change with M404.
	  filament_width_meas = DEFAULT_MEASURED_FILAMENT_DIA;    // Measured filament diameter
uint8_t meas_delay_cm = MEASUREMENT_DELAY_CM,                 // Distance delay setting
		measurement_delay[MAX_MEASUREMENT_DELAY +
												1];         // Ring buffer to delayed measurement. Store extruder factor after subtracting 100
...
{
	planner.buffer_line(destination[X_AXIS], destination[Y_AXIS], destination[Z_AXIS], destination[E_AXIS], fr_mm_s,
						active_extruder);
}
inline void line_to_destination()
{
	line_to_destination(feedrate_mm_s);
}
...
void prepare_uninterpolated_move_to_destination(const float fr_mm_s = 0.0)
{
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		DEBUG_POS("prepare_uninterpolated_move_to_destination", destination);
	}
	#endif
...
void do_blocking_move_to(const float &lx, const float &ly, const float &lz, const float &fr_mm_s/*=0.0*/)
{
	...
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		print_xyz(PSTR(">>> do_blocking_move_to"), NULL, lx, ly, lz);
	}
	#endif
	...
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		DEBUG_POS("set_destination_to_current", destination);
	}
	#endif
	...
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		SERIAL_ECHOLNPGM("<<< do_blocking_move_to");
	}
	#endif
}
...
static void setup_for_endstop_or_probe_move()
{
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		DEBUG_POS("setup_for_endstop_or_probe_move", current_position);
	}
	#endif
...
static void clean_up_after_endstop_or_probe_move()
{
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		DEBUG_POS("clean_up_after_endstop_or_probe_move", current_position);
	}
	#endif
...
bool axis_unhomed_error(const bool x/*=true*/, const bool y/*=true*/, const bool z/*=true*/)
{
	...
	if (xx || yy || zz) {
		...
		if (xx) {
			SERIAL_ECHOPGM(MSG_X);
		}
		if (yy) {
			SERIAL_ECHOPGM(MSG_Y);
		}
		if (zz) {
			SERIAL_ECHOPGM(MSG_Z);
		}
...
#define _AUE_ARGS
	...
	if (_TRIGGERED_WHEN_STOWED_TEST == deploy) {     // closed after the probe specific actions.
		...
		if (!deploy) {
			endstops.enable_z_probe(false);    // Switch off triggered when stowed probes early
		}
...
static float run_z_probe(const bool short_move = true)
{
	...
	if (do_probe_move(-10, Z_PROBE_SPEED_FAST)) {
		return NAN;
	}
	...
	if (DEBUGGING(LEVELING)) {
		SERIAL_ECHOLNPAIR("1st Probe Z:", first_probe_z);
	}
	#endif
	...
	if (do_probe_move(-10 + (short_move ? 0 : -(Z_MAX_LENGTH)), Z_PROBE_SPEED_SLOW)) {
		return NAN;
	}
...
*/
float probe_pt(const float &lx, const float &ly, const bool stow, const uint8_t verbose_level,
			   const bool printable = true)
{
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
...
void set_z_fade_height(const float zfh)
{
	...
	if (level_active) {
		set_bed_leveling_enabled(
			false);    // turn off before changing fade height for proper apply/unapply leveling to maintain current_position
	}
...
*/
static void print_2d_array(const uint8_t sx, const uint8_t sy, const uint8_t precision, float (*fn)(const uint8_t,
						   const uint8_t))
{
...
static void do_homing_move(const AxisEnum axis, const float distance, const float fr_mm_s = 0.0)
{
	...
	planner.buffer_line(delta[A_AXIS], delta[B_AXIS], delta[C_AXIS], current_position[E_AXIS],
						fr_mm_s ? fr_mm_s : homing_feedrate(axis), active_extruder);
	#else
	sync_plan_position();
	current_position[axis] = distance;
	planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS],
						current_position[E_AXIS], fr_mm_s ? fr_mm_s : homing_feedrate(axis), active_extruder);
	#endif
			...
			)
{
	...
	#endif
	if (!retracting) {
		swapping = retracted_swap[active_extruder];
	}
	...
	if (retracting) {
		...
		const float move_e = swapping ? swap_retract_length + swap_retract_recover_length : retract_length +
...
void normalize_mix()
{
	...
	for (uint8_t i = 0; i < MIXING_STEPPERS; i++) {
		mix_total += RECIPROCAL(mixing_factor[i]);
	}
	// Scale all values if they don't add up to ~1.0
	if (!NEAR(mix_total, 1.0)) {
		SERIAL_PROTOCOLLNPGM("Warning: Mix factors must add up to 1.0. Scaling.");
		for (uint8_t i = 0; i < MIXING_STEPPERS; i++) {
			mixing_factor[i] *= mix_total;
		}
	}
}
...
void gcode_get_destination()
{
	LOOP_XYZE(i) {
		if (parser.seen(axis_codes[i])) {
			destination[i] = parser.value_axis_units((AxisEnum)i) + (axis_relative_modes[i] ||
							 relative_mode ? current_position[i] : 0);
		}
		else {
...
inline void gcode_G4()
{
	...
	if (parser.seenval('P')) {
		dwell_ms = parser.value_millis();    // milliseconds to wait
	}
	if (parser.seenval('S')) {
		dwell_ms = parser.value_millis_from_seconds();    // seconds to wait
	}
...
inline bool home_delta()
{
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		DEBUG_POS(">>> home_delta", current_position);
	}
	#endif
	...
	current_position[X_AXIS] = current_position[Y_AXIS] = current_position[Z_AXIS] = (DELTA_HEIGHT + home_offset[Z_AXIS] +
							   10);
...
inline void home_z_safely()
{
	...
	*/
	...
	if (position_is_reachable_xy(destination[X_AXIS], destination[Y_AXIS])) {
		...
		#if ENABLED(DEBUG_LEVELING_FEATURE)
		if (DEBUGGING(LEVELING)) {
			DEBUG_POS("Z_SAFE_HOMING", destination);
		}
		#endif
...
inline void gcode_G28(const bool always_home_all)
{
	...
	#if ENABLED(DEBUG_LEVELING_FEATURE)
	if (DEBUGGING(LEVELING)) {
		SERIAL_ECHOLNPGM("> endstops.enable(true)");
	}
	#endif
	...
	if (home_all || homeZ) {
		...
		#if ENABLED(DEBUG_LEVELING_FEATURE)
		if (DEBUGGING(LEVELING)) {
			DEBUG_POS("> (home_all || homeZ) > final", current_position);
		}
		#endif
	} // home_all || homeZ
	#endif // Z_HOME_DIR < 0
...
void mbl_mesh_report()
{
	...
	SERIAL_PROTOCOLPGM("Z offset: ");
	SERIAL_PROTOCOL_F(mbl.z_offset, 5);
	SERIAL_PROTOCOLLNPGM("\nMeasured points:");
	print_2d_array(GRID_MAX_POINTS_X, GRID_MAX_POINTS_Y, 5,
	[](const uint8_t ix, const uint8_t iy) {
		return mbl.z_values[ix][iy];
	}
				  );
}
	...
	*/
	if (!g29_in_progress) {
		...
		if (verbose_level > 0) {
			...
			if (dryrun) {
				SERIAL_PROTOCOLLNPGM("Running in DRY-RUN mode");
			}
		}
	...
	while (abl_probe_index < abl2) {
		...
		if (position_is_reachable_xy(xProbe, yProbe)) {
			break;
		}
		++abl_probe_index;
	}
	...
	#else // !PROBE_MANUALLY
	{
		...
		for (uint8_t PR_OUTER_VAR = 0; PR_OUTER_VAR < PR_OUTER_END && !isnan(measured_z); PR_OUTER_VAR++) {
			...
			for (int8_t PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; PR_INNER_VAR += inInc) {
				...
				if (!position_is_reachable_by_probe_xy(xProbe, yProbe)) {
					continue;
				}
				#endif
	...
	if (!isnan(measured_z)) {
		...
		*/
		...
		plane_equation_coefficients[0] =
			-lsf_results.A;  // We should be able to eliminate the '-' on these three lines and down below
		...
		if (!dryrun)
			planner.bed_level_matrix = matrix_3x3::create_look_at(
										   vector_3(-plane_equation_coefficients[0], -plane_equation_coefficients[1],
													1)    // We can eliminate the '-' here and up above
									   );
		...
		if (!dryrun) {
			#if ENABLED(DEBUG_LEVELING_FEATURE)
			if (DEBUGGING(LEVELING)) {
				SERIAL_ECHOLNPAIR("G29 uncorrected Z:", current_position[Z_AXIS]);
			}
			#endif
...
inline void gcode_G30()
{
	...
	if (!position_is_reachable_by_probe_xy(xpos, ypos)) {
		return;
	}
...
#define Z0888(I) ZP(a_factor * 8.0 / 9.0, I)
		...
		if (verbose_level != 0) {                                    // !dry run
			if ((zero_std_dev >= test_precision || zero_std_dev <= calibration_precision) &&
				iterations > force_iterations) {  // end iterations
...
#define _GET_MESH_Y(J) mbl.index_to_ypos[J]
		...
		set_destination_to_current();
		if (hasI) {
			destination[X_AXIS] = LOGICAL_X_POSITION(_GET_MESH_X(ix));
		}
		if (hasJ) {
			destination[Y_AXIS] = LOGICAL_Y_POSITION(_GET_MESH_Y(iy));
		}
		if (parser.boolval('P')) {
			if (hasI) {
				destination[X_AXIS] -= X_PROBE_OFFSET_FROM_EXTRUDER;
...
inline void gcode_M0_M1()
{
	...
	if (ms > 0) {
		...
		while (PENDING(millis(), ms) && wait_for_user) {
			idle();
		}
	}
	else {
...
{
	refresh_cmd_timeout();
	while (PENDING(millis(), SPINDLE_LASER_POWERUP_DELAY + previous_cmd_ms)) {
		idle();
	}
}
...
{
	refresh_cmd_timeout();
	while (PENDING(millis(), SPINDLE_LASER_POWERDOWN_DELAY + previous_cmd_ms + 1)) {
		idle();
	}
}
...
inline void ocr_val_mode()
{
	...
	if (SPINDLE_LASER_PWM_INVERT) {
		spindle_laser_power = 255 - spindle_laser_power;
	}
	...
	*/
	...
	if (parser.seen('O')) {
		...
		if (spindle_laser_power == 0) {
			WRITE(SPINDLE_LASER_ENABLE_PIN,
				  !SPINDLE_LASER_ENABLE_INVERT);                                    // turn spindle off (active low)
			delay_for_power_down();
		}
		else {
			int16_t ocr_val = (spindle_laser_power - (SPEED_POWER_INTERCEPT)) * (1.0 /
							  (SPEED_POWER_SLOPE));  // convert RPM to PWM duty cycle
			NOMORE(ocr_val,
				   255);                                                                             // limit to max the Atmel PWM will support
			...
			WRITE(SPINDLE_LASER_ENABLE_PIN,
				  SPINDLE_LASER_ENABLE_INVERT);                                     // turn spindle on (active low)
	...
	#else
	WRITE(SPINDLE_LASER_ENABLE_PIN,
		  SPINDLE_LASER_ENABLE_INVERT); // turn spindle on (active low) if spindle speed option not enabled
	delay_for_power_up();
	#endif
}
...
static void resume_print(const float &load_length = 0, const float &initial_extrude_length = 0,
...
inline void gcode_M23()
{
	...
	for (char *fn = parser.string_arg; *fn; ++fn) if (*fn == ' ') {
			*fn = '\0';
		}
...
static bool pin_is_protected(const int8_t pin)
{
	...
	for (uint8_t i = 0; i < COUNT(sensitive_pins); i++)
		if (pin == (int8_t)pgm_read_byte(&sensitive_pins[i])) {
			return true;
		}
	return false;
}
...
#define PROBE_TEST_PIN Z_MIN_PROBE_PIN
	...
	if (probe_inverting != deploy_state) {
		SERIAL_PROTOCOLLNPGM("WARNING - INVERTING setting probably backwards");
	}
	...
	if (deploy_state != stow_state) {
		...
		for (uint16_t j = 0; j < 500 * 30 && probe_counter == 0 ; j++) {
			...
			if (deploy_state != READ(PROBE_TEST_PIN)) { // probe triggered
				...
				if (probe_counter == 50) {
					...
					SERIAL_PROTOCOLLNPAIR("BLTouch compatible probe detected - pulse width (+/- 4mS): ",
										  probe_counter * 2);    // allow 4 - 100mS pulse
				}
				else {
		...
		if (probe_counter == 0) {
			SERIAL_PROTOCOLLNPGM("trigger not detected");
		}
...
inline void gcode_M43()
{
	...
	if (parser.boolval('W')) {
		...
		for (int8_t pin = first_pin; pin <= last_pin; pin++) {
			if (pin_is_protected(pin) && !ignore_protection) {
				continue;
			}
	...
	do {
		...
		if (wants_to_cool) {
			...
			if (!next_cool_check_ms || ELAPSED(now, next_cool_check_ms)) {
				if (old_temp - temp < MIN_COOLING_SLOPE_DEG) {
					break;
				}
...
#define TEMP_BED_CONDITIONS (wants_to_cool ? thermalManager.isCoolingBed() : thermalManager.isHeatingBed())
	...
	do {
		...
		if (wants_to_cool) {
			...
			if (!next_cool_check_ms || ELAPSED(now, next_cool_check_ms)) {
				if (old_temp - temp < MIN_COOLING_SLOPE_DEG_BED) {
					break;
				}
...
inline void gcode_M18_M84()
{
	if (parser.seenval('S')) {
		...
		#if ENABLED(AUTO_BED_LEVELING_UBL) && ENABLED(LCDKIND_CHARACTER)  // Only needed with an LCD
	...
	LOOP_XYZE(i) {
		if (parser.seen(axis_codes[i])) {
			if (i == E_AXIS) {
				...
				if (value < 20.0) {
					float factor = planner.axis_steps_per_mm[E_AXIS + TARGET_EXTRUDER] /
...
inline void gcode_M202()
{
	LOOP_XYZE(i) {
		if (parser.seen(axis_codes[i])) {
			axis_travel_steps_per_sqr_second[i] = parser.value_axis_units((AxisEnum)i) * planner.axis_steps_per_mm[i];
		}
	}
}
#endif
...
{
	if (parser.seen('S')) {
		planner.min_feedrate_mm_s = parser.value_linear_units();
	}
	if (parser.seen('T')) {
		planner.min_travel_feedrate_mm_s = parser.value_linear_units();
	}
	if (parser.seen('B')) {
		planner.min_segment_time = parser.value_millis();
	}
	if (parser.seen('X')) {
...
inline void gcode_M206()
{
	...
	#if ENABLED(MORGAN_SCARA)
	if (parser.seen('T')) {
		set_home_offset(A_AXIS, parser.value_linear_units());    // Theta
	}
	if (parser.seen('P')) {
		set_home_offset(B_AXIS, parser.value_linear_units());    // Psi
	}
	#endif
...
{
	if (parser.seen('Z')) {
		z_endstop_adj = parser.value_linear_units();
	}
...
{
	if (parser.seen('S')) {
		retract_length = parser.value_axis_units(E_AXIS);
	}
	if (parser.seen('F')) {
		retract_feedrate_mm_s = MMM_TO_MMS(parser.value_axis_units(E_AXIS));
	}
	if (parser.seen('Z')) {
		retract_zlift = parser.value_linear_units();
	}
	if (parser.seen('W')) {
		swap_retract_length = parser.value_axis_units(E_AXIS);
	}
}
...
{
	if (parser.seen('S')) {
		retract_recover_length = parser.value_axis_units(E_AXIS);
	}
	if (parser.seen('F')) {
		retract_recover_feedrate_mm_s = MMM_TO_MMS(parser.value_axis_units(E_AXIS));
	}
	if (parser.seen('R')) {
		swap_retract_recover_feedrate_mm_s = MMM_TO_MMS(parser.value_axis_units(E_AXIS));
	}
	if (parser.seen('W')) {
		swap_retract_recover_length = parser.value_axis_units(E_AXIS);
	}
}
...
inline void gcode_M211()
{
	...
	#if HAS_SOFTWARE_ENDSTOPS
	if (parser.seen('S')) {
		soft_endstops_enabled = parser.value_bool();
	}
...
{
	if (get_target_extruder_from_command(218) || target_extruder == 0) {
		return;
	}
...
inline void gcode_M301()
{
	...
	if (e < HOTENDS) { // catch bad input value
		...
		if (parser.seen('I')) {
			PID_PARAM(Ki, e) = scalePID_i(parser.value_float());
		}
		if (parser.seen('D')) {
			PID_PARAM(Kd, e) = scalePID_d(parser.value_float());
		}
		#if ENABLED(PID_EXTRUSION_SCALING)
		if (parser.seen('C')) {
			PID_PARAM(Kc, e) = parser.value_float();
		}
		if (parser.seen('L')) {
			lpq_len = parser.value_float();
		}
...
{
	if (parser.seen('P')) {
		thermalManager.bedKp = parser.value_float();
	}
	if (parser.seen('I')) {
		thermalManager.bedKi = scalePID_i(parser.value_float());
	}
	if (parser.seen('D')) {
		thermalManager.bedKd = scalePID_d(parser.value_float());
	}
...
{
	if (parser.seen('C')) {
		set_lcd_contrast(parser.value_int());
	}
...
inline void gcode_M421()
{
	...
	const int8_t ix = hasI ? parser.value_int() : hasX ? mbl.probe_index_x(RAW_X_POSITION(
						  parser.value_linear_units())) : -1;
	const bool hasY = parser.seen('Y'), hasJ = parser.seen('J');
	const int8_t iy = hasJ ? parser.value_int() : hasY ? mbl.probe_index_y(RAW_Y_POSITION(
						  parser.value_linear_units())) : -1;
...
inline void gcode_M421()
{
	...
	if (hasC) {
		const mesh_index_pair location = ubl.find_closest_mesh_point_of_type(REAL, current_position[X_AXIS],
										 current_position[Y_AXIS], USE_NOZZLE_AS_REFERENCE, NULL, false);
		ix = location.x_index;
		iy = location.y_index;
	}
...
inline void gcode_M428()
{
	...
	LOOP_XYZ(i) {
		if (axis_homed[i]) {
			const float base = (current_position[i] > (soft_endstop_min[i] + soft_endstop_max[i]) * 0.5) ? base_home_pos((
								   AxisEnum)i) : 0,
...
inline void gcode_M851()
{
	...
	if (parser.seen('Z')) {
		...
		if (WITHIN(value, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX)) {
			...
			SERIAL_ECHOPGM(MSG_Z_MIN " " STRINGIFY(Z_PROBE_OFFSET_RANGE_MIN) " " MSG_Z_MAX " " STRINGIFY(
...
inline void gcode_M605()
{
	...
	switch (dual_x_carriage_mode) {
		...
		case DXC_DUPLICATION_MODE:
			if (parser.seen('X')) {
				duplicate_extruder_x_offset = max(parser.value_linear_units(), X2_MIN_POS - x_home_pos(0));
			}
			if (parser.seen('R')) {
...
inline void gcode_M906()
{
	...
	#if ENABLED(X_IS_TMC2130)
	if (values[X_AXIS]) {
		tmc2130_set_current(stepperX, 'X', values[X_AXIS]);
	}
	else {
		tmc2130_get_current(stepperX, 'X');
	}
	#endif
	#if ENABLED(Y_IS_TMC2130)
	if (values[Y_AXIS]) {
		tmc2130_set_current(stepperY, 'Y', values[Y_AXIS]);
	}
	else {
		tmc2130_get_current(stepperY, 'Y');
	}
	#endif
	#if ENABLED(Z_IS_TMC2130)
	if (values[Z_AXIS]) {
		tmc2130_set_current(stepperZ, 'Z', values[Z_AXIS]);
	}
	else {
		tmc2130_get_current(stepperZ, 'Z');
	}
	#endif
	#if ENABLED(E0_IS_TMC2130)
	if (values[E_AXIS]) {
		tmc2130_set_current(stepperE0, 'E', values[E_AXIS]);
	}
	else {
		tmc2130_get_current(stepperE0, 'E');
	}
	#endif

	#if ENABLED(AUTOMATIC_CURRENT_CONTROL)
	if (parser.seen('S')) {
		auto_current_control = parser.value_bool();
	}
	#endif
}
...
{
	const bool reportX = parser.seen('X'), reportY = parser.seen('Y'), reportZ = parser.seen('Z'),
			   reportE = parser.seen('E'),
			   reportAll = (!reportX && !reportY && !reportZ && !reportE) || (reportX && reportY && reportZ && reportE);
	#if ENABLED(X_IS_TMC2130)
	if (reportX || reportAll) {
		tmc2130_report_otpw(stepperX, 'X');
	}
	#endif
	#if ENABLED(Y_IS_TMC2130)
	if (reportY || reportAll) {
		tmc2130_report_otpw(stepperY, 'Y');
	}
	#endif
	#if ENABLED(Z_IS_TMC2130)
	if (reportZ || reportAll) {
		tmc2130_report_otpw(stepperZ, 'Z');
	}
	#endif
	#if ENABLED(E0_IS_TMC2130)
	if (reportE || reportAll) {
...
inline void gcode_M912()
{
	...
	#if ENABLED(X_IS_TMC2130)
	if (clearX || clearAll) {
		tmc2130_clear_otpw(stepperX, 'X');
	}
	#endif
	#if ENABLED(Y_IS_TMC2130)
	if (clearY || clearAll) {
		tmc2130_clear_otpw(stepperY, 'Y');
	}
	#endif
	#if ENABLED(Z_IS_TMC2130)
	if (clearZ || clearAll) {
		tmc2130_clear_otpw(stepperZ, 'Z');
	}
	#endif
	#if ENABLED(E0_IS_TMC2130)
	if (clearE || clearAll) {
		tmc2130_clear_otpw(stepperE0, 'E');
	}
	#endif
}
...
inline void gcode_M913()
{
	...
	#if ENABLED(X_IS_TMC2130)
	if (values[X_AXIS]) {
		tmc2130_set_pwmthrs(stepperX, 'X', values[X_AXIS], planner.axis_steps_per_mm[X_AXIS]);
	}
	else {
		tmc2130_get_pwmthrs(stepperX, 'X', planner.axis_steps_per_mm[X_AXIS]);
	}
	#endif
	#if ENABLED(Y_IS_TMC2130)
	if (values[Y_AXIS]) {
		tmc2130_set_pwmthrs(stepperY, 'Y', values[Y_AXIS], planner.axis_steps_per_mm[Y_AXIS]);
	}
	else {
		tmc2130_get_pwmthrs(stepperY, 'Y', planner.axis_steps_per_mm[Y_AXIS]);
	}
	#endif
	#if ENABLED(Z_IS_TMC2130)
	if (values[Z_AXIS]) {
		tmc2130_set_pwmthrs(stepperZ, 'Z', values[Z_AXIS], planner.axis_steps_per_mm[Z_AXIS]);
	}
	else {
		tmc2130_get_pwmthrs(stepperZ, 'Z', planner.axis_steps_per_mm[Z_AXIS]);
	}
	#endif
	#if ENABLED(E0_IS_TMC2130)
	if (values[E_AXIS]) {
		tmc2130_set_pwmthrs(stepperE0, 'E', values[E_AXIS], planner.axis_steps_per_mm[E_AXIS]);
	}
	else {
		tmc2130_get_pwmthrs(stepperE0, 'E', planner.axis_steps_per_mm[E_AXIS]);
	}
	#endif
}
...
inline void gcode_M914()
{
	...
	if (parser.seen(axis_codes[X_AXIS])) {
		tmc2130_set_sgt(stepperX, 'X', parser.value_int());
	}
	else {
		tmc2130_get_sgt(stepperX, 'X');
	}
	#endif
	#if ENABLED(Y_IS_TMC2130)
	if (parser.seen(axis_codes[Y_AXIS])) {
		tmc2130_set_sgt(stepperY, 'Y', parser.value_int());
	}
	else {
		tmc2130_get_sgt(stepperY, 'Y');
	}
	#endif
}
...
inline void gcode_M907()
{
	...
	LOOP_XYZE(i) if (parser.seen(axis_codes[i])) {
		stepper.digipot_current(i, parser.value_int());
	}
	if (parser.seen('B')) {
		stepper.digipot_current(4, parser.value_int());
	}
	if (parser.seen('S')) for (uint8_t i = 0; i <= 4; i++) {
			stepper.digipot_current(i, parser.value_int());
		}
	...
	if (parser.seen('X')) {
		stepper.digipot_current(0, parser.value_int());
	}
	#endif
	#if PIN_EXISTS(MOTOR_CURRENT_PWM_Z)
	if (parser.seen('Z')) {
		stepper.digipot_current(1, parser.value_int());
	}
	#endif
	#if PIN_EXISTS(MOTOR_CURRENT_PWM_E)
	if (parser.seen('E')) {
		stepper.digipot_current(2, parser.value_int());
	}
	#endif
	...
	LOOP_XYZE(i) if (parser.seen(axis_codes[i])) {
		digipot_i2c_set_current(i, parser.value_float());
	}
	// for each additional extruder (named B,C,D,E..., channels 4,5,6,7...)
	for (uint8_t i = NUM_AXIS; i < DIGIPOT_I2C_NUM_CHANNELS; i++) if (parser.seen('B' + i - (NUM_AXIS))) {
			digipot_i2c_set_current(i, parser.value_float());
		}
	#endif
	...
	if (parser.seen('S')) {
		...
		for (uint8_t i = 0; i <= 4; i++) {
			dac_current_percent(i, dac_percent);
		}
	}
	LOOP_XYZE(i) if (parser.seen(axis_codes[i])) {
		dac_current_percent(i, parser.value_float());
	}
	#endif
}
...
{
	if (parser.seen('S')) for (int i = 0; i <= 4; i++) {
			stepper.microstep_mode(i, parser.value_byte());
		}
	LOOP_XYZE(i) if (parser.seen(axis_codes[i])) {
		stepper.microstep_mode(i, parser.value_byte());
	}
	if (parser.seen('B')) {
		stepper.microstep_mode(4, parser.value_byte());
	}
...
inline void gcode_M351()
{
	if (parser.seenval('S')) switch (parser.value_byte()) {
			case 1:
				LOOP_XYZE(i) if (parser.seenval(axis_codes[i])) {
					stepper.microstep_ms(i, parser.value_byte(), -1);
				}
				if (parser.seenval('B')) {
					stepper.microstep_ms(4, parser.value_byte(), -1);
				}
				break;
			case 2:
				LOOP_XYZE(i) if (parser.seenval(axis_codes[i])) {
					stepper.microstep_ms(i, -1, parser.value_byte());
				}
				if (parser.seenval('B')) {
					stepper.microstep_ms(4, -1, parser.value_byte());
				}
				break;
		}
	stepper.microstep_readings();
...
void tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool no_move/*=false*/)
{
	...
	if (tmp_extruder != active_extruder) {
		...
		if (DEBUGGING(LEVELING)) {
			...
			switch (dual_x_carriage_mode) {
				case DXC_FULL_CONTROL_MODE:
					SERIAL_ECHOLNPGM("DXC_FULL_CONTROL_MODE");
					break;
				case DXC_AUTO_PARK_MODE:
					SERIAL_ECHOLNPGM("DXC_AUTO_PARK_MODE");
					break;
				case DXC_DUPLICATION_MODE:
					SERIAL_ECHOLNPGM("DXC_DUPLICATION_MODE");
					break;
			}
		}
		#endif
...
#define DONT_SWITCH (SWITCHING_EXTRUDER_SERVO_NR == SWITCHING_NOZZLE_SERVO_NR)
		...
		*/
		...
		#if ENABLED(DEBUG_LEVELING_FEATURE)
		if (DEBUGGING(LEVELING)) {
			offset_vec.debug(PSTR("offset_vec (AFTER)"));
		}
		#endif
		...
		if (leveling_is_active()) {
			#if ENABLED(DEBUG_LEVELING_FEATURE)
			if (DEBUGGING(LEVELING)) {
				SERIAL_ECHOPAIR("Z before MBL: ", current_position[Z_AXIS]);
			}
			#endif
		...
		#if ENABLED(DEBUG_LEVELING_FEATURE)
		if (DEBUGGING(LEVELING)) {
			DEBUG_POS("Sync After Toolchange", current_position);
		}
		#endif
...
void process_next_command()
{
	...
	if (DEBUGGING(ECHO)) {
		...
		M100_dump_routine("   Command Queue:", (const char *)command_queue,
...
void recalc_delta_settings(float radius, float diagonal_rod)
{
	...
	delta_tower[A_AXIS][X_AXIS] = cos(RADIANS(210 + delta_tower_angle_trim[A_AXIS])) * (radius +
								  trt[A_AXIS]); // front left tower
	delta_tower[A_AXIS][Y_AXIS] = sin(RADIANS(210 + delta_tower_angle_trim[A_AXIS])) * (radius + trt[A_AXIS]);
	delta_tower[B_AXIS][X_AXIS] = cos(RADIANS(330 + delta_tower_angle_trim[B_AXIS])) * (radius +
								  trt[B_AXIS]); // front right tower
...
inline bool prepare_kinematic_move_to(float ltarget[XYZE])
{
	...
	if (UNEAR_ZERO(cartesian_mm)) {
		cartesian_mm = FABS(difference[E_AXIS]);
	}
	...
	for (uint16_t s = segments + 1; --s;) {
		...
		planner.buffer_line(delta[A_AXIS], delta[B_AXIS], delta[C_AXIS], logical[E_AXIS], max(adiff, bdiff) * feed_factor,
							active_extruder);
		oldA = delta[A_AXIS];
		oldB = delta[B_AXIS];
		#else
		...
		switch (dual_x_carriage_mode) {
			...
			case DXC_DUPLICATION_MODE:
				if (active_extruder == 0) {
					...
					#if ENABLED(DEBUG_LEVELING_FEATURE)
					if (DEBUGGING(LEVELING)) {
						SERIAL_ECHOLNPGM("Set extruder_duplication_enabled\nClear active_extruder_parked");
					}
					#endif
				}
				else {
...
void stop()
{
	...
	#if ENABLED(PROBING_FANS_OFF)
	if (fans_paused) {
		fans_pause(false);    // put things back the way they were
	}
	#endif
...
#define CUSTOM_BOOTSCREEN_TIMEOUT 2500
	...
	#if ENABLED(LCDKIND_GRAPHIC)                           // On DOGM the first bootscreen is already drawn
	...
	#elif ENABLED(LCDKIND_CHARACTER)
	lcd_bootscreen();
	#if DISABLED(SDSUPPORT)
	lcd_init();
	#endif
