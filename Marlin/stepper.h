...
public:
	...
	//
	#if IS_SCARA
	static FORCE_INLINE float get_axis_position_degrees(AxisEnum axis)
	{
		return get_axis_position_mm(axis);
	}
	#endif
	...
	#if ENABLED(Z_DUAL_ENDSTOPS)
	static FORCE_INLINE void set_homing_flag(const bool state)
	{
	...
	static void babystep(const AxisEnum axis,
						 const bool direction); // perform a short step with a single stepper motor, outside of any convention
	#endif
...
private:
	...
	static FORCE_INLINE void trapezoid_generator_reset()
	{
		...
		if (current_block->use_advance_lead) {
			current_estep_rate[current_block->active_extruder] = ((unsigned long)acc_step_rate *
					current_block->abs_adv_steps_multiplier8) >> 17;
