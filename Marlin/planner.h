...
public:
	...
	*/
	...
	static bool is_full()
	{
		return (block_buffer_tail == BLOCK_MOD(block_buffer_head + 1));
	}
...
#define ARG_Z float lz
	...
	*/
	...
	static void apply_leveling(float logical[XYZ])
	{
		apply_leveling(logical[X_AXIS], logical[Y_AXIS], logical[Z_AXIS]);
	}
...
#define ARG_Z const float &lz
	...
	*/
	static void _buffer_line(const float &a, const float &b, const float &c, const float &e, float fr_mm_s,
	...
	*/
	...
	static FORCE_INLINE void set_z_position_mm(const float &z)
	{
		set_position_mm(Z_AXIS, z);
	}
	...
	*/
	static block_t *get_current_block()
	{
		if (blocks_queued()) {
			...
			#if ENABLED(LCDKIND_CHARACTER)
			block_buffer_runtime_us -=
				block->segment_time; //We can't be sure how long an active block will take, so don't count it.
			#endif
...
private:
	...
	*/
	static float intersection_distance(const float &initial_rate, const float &final_rate, const float &accel,
