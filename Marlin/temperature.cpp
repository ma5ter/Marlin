...
#else
static void *heater_ttbl_map[HOTENDS] = ARRAY_BY_HOTENDS((void *)HEATER_0_TEMPTABLE, (void *)HEATER_1_TEMPTABLE,
...
int16_t Temperature::minttemp_raw[HOTENDS] = ARRAY_BY_HOTENDS(HEATER_0_RAW_LO_TEMP, HEATER_1_RAW_LO_TEMP,
		HEATER_2_RAW_LO_TEMP, HEATER_3_RAW_LO_TEMP, HEATER_4_RAW_LO_TEMP),
		Temperature::maxttemp_raw[HOTENDS] = ARRAY_BY_HOTENDS(HEATER_0_RAW_HI_TEMP, HEATER_1_RAW_HI_TEMP, HEATER_2_RAW_HI_TEMP,
				HEATER_3_RAW_HI_TEMP, HEATER_4_RAW_HI_TEMP),
...
#define _HOTEND_TEST     e == active_extruder
	...
	dTerm[HOTEND_INDEX] = K2 * PID_PARAM(Kd,
										 HOTEND_INDEX) * (current_temperature[HOTEND_INDEX] - temp_dState[HOTEND_INDEX]) + K1 * dTerm[HOTEND_INDEX];
...
void Temperature::manage_heater()
{
	...
	HOTEND_LOOP() {
		...
		thermal_runaway_protection(&thermal_runaway_state_machine[e], &thermal_runaway_timer[e], current_temperature[e],
								   target_temperature[e], e, THERMAL_PROTECTION_PERIOD, THERMAL_PROTECTION_HYSTERESIS);
		#endif

		soft_pwm_amount[e] = (current_temperature[e] > minttemp[e] || is_preheating(e)) &&
							 current_temperature[e] < maxttemp[e] ? (int)get_pid_output(e) >> 1 : 0;
	...
	thermal_runaway_protection(&thermal_runaway_bed_state_machine, &thermal_runaway_bed_timer, current_temperature_bed,
							   target_temperature_bed, -1, THERMAL_PROTECTION_BED_PERIOD, THERMAL_PROTECTION_BED_HYSTERESIS);
	#endif
...
int Temperature::widthFil_to_size_ratio()
{
	...
	if (temp < MEASURED_LOWER_LIMIT) {
		...
		NOMORE(temp, MEASURED_UPPER_LIMIT);
	}
...
void Temperature::thermal_runaway_protection(Temperature::TRState *state, millis_t *timer, float current, float target,
		int heater_id, int period_seconds, int hysteresis_degc)
{
...
void endstop_monitor()
{
	...
	#if HAS_X_MIN
	if (READ(X_MIN_PIN)) {
		SBI(current_endstop_bits_local, X_MIN);
	}
	#endif
	#if HAS_X_MAX
	if (READ(X_MAX_PIN)) {
		SBI(current_endstop_bits_local, X_MAX);
	}
	#endif
	#if HAS_Y_MIN
	if (READ(Y_MIN_PIN)) {
		SBI(current_endstop_bits_local, Y_MIN);
	}
	#endif
	#if HAS_Y_MAX
	if (READ(Y_MAX_PIN)) {
		SBI(current_endstop_bits_local, Y_MAX);
	}
	#endif
	#if HAS_Z_MIN
	if (READ(Z_MIN_PIN)) {
		SBI(current_endstop_bits_local, Z_MIN);
	}
	#endif
	#if HAS_Z_MAX
	if (READ(Z_MAX_PIN)) {
		SBI(current_endstop_bits_local, Z_MAX);
	}
	#endif
	#if HAS_Z_MIN_PROBE_PIN
	if (READ(Z_MIN_PROBE_PIN)) {
		SBI(current_endstop_bits_local, Z_MIN_PROBE);
	}
	#endif
	#if HAS_Z2_MIN
	if (READ(Z2_MIN_PIN)) {
		SBI(current_endstop_bits_local, Z2_MIN);
	}
	#endif
	#if HAS_Z2_MAX
	if (READ(Z2_MAX_PIN)) {
		SBI(current_endstop_bits_local, Z2_MAX);
	}
	#endif
	...
	if (endstop_change) {
		#if HAS_X_MIN
		if (TEST(endstop_change, X_MIN)) {
			SERIAL_PROTOCOLPAIR("X_MIN:", !!TEST(current_endstop_bits_local, X_MIN));
		}
		#endif
		#if HAS_X_MAX
		if (TEST(endstop_change, X_MAX)) {
			SERIAL_PROTOCOLPAIR("  X_MAX:", !!TEST(current_endstop_bits_local, X_MAX));
		}
		#endif
		#if HAS_Y_MIN
		if (TEST(endstop_change, Y_MIN)) {
			SERIAL_PROTOCOLPAIR("  Y_MIN:", !!TEST(current_endstop_bits_local, Y_MIN));
		}
		#endif
		#if HAS_Y_MAX
		if (TEST(endstop_change, Y_MAX)) {
			SERIAL_PROTOCOLPAIR("  Y_MAX:", !!TEST(current_endstop_bits_local, Y_MAX));
		}
		#endif
		#if HAS_Z_MIN
		if (TEST(endstop_change, Z_MIN)) {
			SERIAL_PROTOCOLPAIR("  Z_MIN:", !!TEST(current_endstop_bits_local, Z_MIN));
		}
		#endif
		#if HAS_Z_MAX
		if (TEST(endstop_change, Z_MAX)) {
			SERIAL_PROTOCOLPAIR("  Z_MAX:", !!TEST(current_endstop_bits_local, Z_MAX));
		}
		#endif
		#if HAS_Z_MIN_PROBE_PIN
		if (TEST(endstop_change, Z_MIN_PROBE)) {
			SERIAL_PROTOCOLPAIR("  PROBE:", !!TEST(current_endstop_bits_local, Z_MIN_PROBE));
		}
		#endif
		#if HAS_Z2_MIN
		if (TEST(endstop_change, Z2_MIN)) {
			SERIAL_PROTOCOLPAIR("  Z2_MIN:", !!TEST(current_endstop_bits_local, Z2_MIN));
		}
		#endif
		#if HAS_Z2_MAX
		if (TEST(endstop_change, Z2_MAX)) {
			SERIAL_PROTOCOLPAIR("  Z2_MAX:", !!TEST(current_endstop_bits_local, Z2_MAX));
		}
		#endif
		SERIAL_PROTOCOLPGM("\n\n");
...
#define ISR_STATICS(n) static uint8_t soft_pwm_count_ ## n = 0
	...
	*/
	if (pwm_count_tmp >= 127) {
		...
		#if HOTENDS > 1
		if (soft_pwm_count_1 <= pwm_count_tmp) {
			WRITE_HEATER_1(LOW);
		}
		#if HOTENDS > 2
		if (soft_pwm_count_2 <= pwm_count_tmp) {
			WRITE_HEATER_2(LOW);
		}
		#if HOTENDS > 3
		if (soft_pwm_count_3 <= pwm_count_tmp) {
			WRITE_HEATER_3(LOW);
		}
		#if HOTENDS > 4
		if (soft_pwm_count_4 <= pwm_count_tmp) {
			WRITE_HEATER_4(LOW);
		}
		#endif // HOTENDS > 4
		#endif // HOTENDS > 3
...
#define PWM_OFF_ROUTINE(NR) \
	...
	if (((pwm_count >> SOFT_PWM_SCALE) & 0x3F) == 0) {
		...
		#if HOTENDS > 4
		if (state_timer_heater_4 > 0) {
			state_timer_heater_4--;
		}
		#endif // HOTENDS > 4
		#endif // HOTENDS > 3
		#endif // HOTENDS > 2
...
#define START_ADC(pin) ADCSRB = 0; SET_ADMUX_ADCSRA(pin)
	...
	if (!adc_sensor_state && ++temp_count >= OVERSAMPLENR) { // 10 * 16 * 1/(16000000/64/256)  = 164ms.
		...
		#if ENABLED(FILAMENT_WIDTH_SENSOR)
		current_raw_filwidth = raw_filwidth_value >>
							   10;  // Divide to get to 0-16384 range since we used 1/128 IIR filter approach
		#endif
...
#define GEBED >=
	...
	if (endstop_monitor_flag) {
		...
		if (!endstop_monitor_count) {
			endstop_monitor();    // report changes in endstop status
		}
	}
	#endif
