...
private:
	...
	static void probe_entire_mesh(const float &lx, const float &ly, const bool do_ubl_mesh_map, const bool stow_probe,
								  bool do_furthest);
	...
	static void print_line_from_here_to_there(const float &, const float &, const float &, const float &, const float &,
			const float &);
	static void move_to(const float &, const float &, const float &, const float &);
	inline static void move_to(const float where[XYZE], const float &de)
	{
...
public:
	...
	static mesh_index_pair find_closest_mesh_point_of_type(const MeshPointType, const float &, const float &, const bool,
			uint16_t[16], bool);
	static void reset();
	...
	FORCE_INLINE static void set_z(const int8_t px, const int8_t py, const float &z)
	{
		z_values[px][py] = z;
	}
...
#if ENABLED(UBL_G26_MESH_VALIDATION)
FORCE_INLINE void gcode_G26()
{
	ubl.G26();
}
#endif

FORCE_INLINE void gcode_G29()
{
	ubl.G29();
}
