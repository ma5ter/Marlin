...
#if ENABLED(AUTO_BED_LEVELING_UBL) && ENABLED(USE_CONTROLLER)
...
#define EXTRA_CYCLES_XYZE (STEP_PULSE_CYCLES - (CYCLES_EATEN_XYZE))
	...
	if (step_events_completed <= (uint32_t)current_block->accelerate_until) {
		...
		if (current_block->use_advance_lead) {
			#if ENABLED(MIXING_EXTRUDER)
			MIXING_STEPPERS_LOOP(j)
			current_estep_rate[j] = ((uint32_t)acc_step_rate * current_block->abs_adv_steps_multiplier8 *
									 current_block->step_event_count / current_block->mix_event_count[j]) >> 17;
			#else
		...
		if (current_block->use_advance_lead) {
			#if ENABLED(MIXING_EXTRUDER)
			MIXING_STEPPERS_LOOP(j)
			current_estep_rate[j] = ((uint32_t)step_rate * current_block->abs_adv_steps_multiplier8 *
									 current_block->step_event_count / current_block->mix_event_count[j]) >> 17;
			#else
...
#define E_AXIS_INIT(NUM) AXIS_INIT(E## NUM, E)
	...
	#if ENABLED(ADVANCE) || ENABLED(LIN_ADVANCE)
	for (uint8_t i = 0; i < COUNT(e_steps); i++) {
		e_steps[i] = 0;
	}
	#if ENABLED(LIN_ADVANCE)
