...
void unified_bed_leveling::line_to_destination_cartesian(const float &feed_rate, uint8_t extruder)
{
	...
	*/
	...
	FINAL_MOVE:
		...
		*/
		...
		float z0 = cell_dest_yi < GRID_MAX_POINTS_Y - 1 ? (z1 + (z2 - z1) * yratio) * fade_scaling_factor_for_z(
					   end[Z_AXIS]) : 0.0;
		...
		while (current_xi != cell_dest_xi + left_flag) {
			...
			*/
			if (x != start[X_AXIS]) {
				if (!inf_normalized_flag) {
					on_axis_distance = use_x_dist ? x - start[X_AXIS] : y - start[Y_AXIS];
					e_position = start[E_AXIS] + on_axis_distance *
								 e_normalized_dist;  // is based on X or Y because this is a horizontal move
					z_position = start[Z_AXIS] + on_axis_distance * z_normalized_dist;
				}
				else {
					e_position = end[E_AXIS];
	...
	*/
	...
	while (xi_cnt > 0 || yi_cnt > 0) {
		...
		if (xi_cnt < 0 || yi_cnt < 0) {
			break;    // we've gone too far, so exit the loop and move on to FINAL_MOVE
		}
	}
...
bool _O2 unified_bed_leveling::prepare_segmented_line_to(const float ltarget[XYZE], const float &feedrate)
{
	...
	#if IS_KINEMATIC
	const float seconds = cartesian_xy_mm /
						  feedrate;                                  // seconds to move xy distance at requested rate
	uint16_t segments = lroundf(delta_segments_per_second *
								seconds),                  // preferred number of segments for distance @ feedrate
						seglimit = lroundf(cartesian_xy_mm * (1.0 /
										   (DELTA_SEGMENT_MIN_LENGTH))); // number of segments at minimum segment length
	NOMORE(segments,
		   seglimit);                                                        // limit to minimum segment length (fewer segments)
	#else
	...
	for (;;) { // for each mesh cell encountered during the move
		...
		if (isnan(z_x0y0)) {
			z_x0y0 = 0;    // ideally activating state.active (G29 A)
		}
		if (isnan(z_x1y0)) {
			z_x1y0 = 0;    //   should refuse if any invalid mesh points
		}
		if (isnan(z_x0y1)) {
			z_x0y1 = 0;    //   in order to avoid isnan tests per cell,
		}
		if (isnan(z_x1y1)) {
			z_x1y1 = 0;    //   thus guessing zero for undefined points
		}
