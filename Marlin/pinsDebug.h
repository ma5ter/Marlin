...
{
	return *portModeRegister(digitalPinToPort_DEBUG(pin)) & digitalPinToBitMask_DEBUG(pin);
}
#else
#define digitalPinToTimer_DEBUG(p) digitalPinToTimer(p)
#define digitalPinToBitMask_DEBUG(p) digitalPinToBitMask(p)
...
static void print_is_also_tied()
{
	SERIAL_PROTOCOLPGM(" is also tied to this pin");
	SERIAL_PROTOCOL_SP(14);
}
...
void timer_prefix(uint8_t T, char L, uint8_t N)    // T - timer    L - pwm  N - WGM bit layout
{
	...
	if (N == 3) {
		if (WGM == 0 || WGM == 2 || WGM ==  4 || WGM ==  6) {
			err_is_counter();
		}
	}
	else        {
		if (WGM == 0 || WGM == 4 || WGM == 12 || WGM == 13) {
			err_is_counter();
		}
	}
	if (TEST(*TMSK, OCIE)) {
			...
			if (extended) {
				if (pin_is_protected(pin) && !ignore) {
					...
					#endif
					{
						...
						if (!multi_name_pin && extended) {
							pwm_details(pin);    // report PWM capabilities only on the first pass & only if doing an extended report
						}
					}
				}
			}
