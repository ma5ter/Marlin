...
void Endstops::test_dual_z_endstops(const EndstopEnum es1, const EndstopEnum es2)
{
	...
	if (z_test && stepper.current_block->steps[Z_AXIS] > 0) {
		...
		if (!stepper.performing_homing ||
			(z_test == 0x3)) { //if not performing home or if both endstops were trigged during homing...
...
#define UPDATE_ENDSTOP(AXIS,MINMAX) do { \
	...
	if (G38_move) {
		...
		if (TEST_ENDSTOP(_ENDSTOP(Z, MIN_PROBE))) {
			if (stepper.current_block->steps[_AXIS(X)] > 0) {
				_ENDSTOP_HIT(X, MIN);
				stepper.endstop_triggered(_AXIS(X));
			}
			else if (stepper.current_block->steps[_AXIS(Y)] > 0) {
				_ENDSTOP_HIT(Y, MIN);
				stepper.endstop_triggered(_AXIS(Y));
			}
			else if (stepper.current_block->steps[_AXIS(Z)] > 0) {
				_ENDSTOP_HIT(Z, MIN);
