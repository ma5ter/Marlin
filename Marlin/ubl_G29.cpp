...
void unified_bed_leveling::G29()
{
	...
	if (g29_parameter_parsing()) {
		return;    // abort if parsing the simple parameters causes a problem,
	}
	...
	if (parser.seen('I')) {
		...
		if (g29_repetition_cnt >= GRID_MAX_POINTS) {
			...
			while (g29_repetition_cnt--) {
				...
				const mesh_index_pair location = find_closest_mesh_point_of_type(REAL, g29_x_pos, g29_y_pos, USE_NOZZLE_AS_REFERENCE,
												 NULL, false);
	...
	if (parser.seen('J')) {
		if (g29_grid_size) {  // if not 0 it is a normal n x n grid being probed
			...
			float z3, z2, z1 = probe_pt(LOGICAL_X_POSITION(UBL_PROBE_PT_1_X), LOGICAL_Y_POSITION(UBL_PROBE_PT_1_Y), false,
										g29_verbose_level);
			if (!isnan(z1)) {
			...
			save_ubl_active_state_and_disable();
			z1 -= get_z_correction(LOGICAL_X_POSITION(UBL_PROBE_PT_1_X),
								   LOGICAL_Y_POSITION(UBL_PROBE_PT_1_Y)) /* + zprobe_zoffset */ ;
			z2 -= get_z_correction(LOGICAL_X_POSITION(UBL_PROBE_PT_2_X),
								   LOGICAL_Y_POSITION(UBL_PROBE_PT_2_Y)) /* + zprobe_zoffset */ ;
			z3 -= get_z_correction(LOGICAL_X_POSITION(UBL_PROBE_PT_3_X),
								   LOGICAL_Y_POSITION(UBL_PROBE_PT_3_Y)) /* + zprobe_zoffset */ ;
				...
				if (g29_c_flag) {
					if (g29_repetition_cnt >= GRID_MAX_POINTS) {
						...
						while (g29_repetition_cnt--) {  // this only populates reachable mesh points near
							const mesh_index_pair location = find_closest_mesh_point_of_type(INVALID, g29_x_pos, g29_y_pos, USE_NOZZLE_AS_REFERENCE,
															 NULL, false);
							if (location.x_index < 0) {
...
*/
void unified_bed_leveling::probe_entire_mesh(const float &lx, const float &ly, const bool do_ubl_mesh_map,
		const bool stow_probe, bool close_or_far)
{
	...
	do {
		...
		if (location.x_index >= 0) {    // mesh point found and is reachable by probe
			...
			const float measured_z = probe_pt(LOGICAL_X_POSITION(rawx), LOGICAL_Y_POSITION(rawy), stow_probe,
											  g29_verbose_level); // TODO: Needs error handling
...
float unified_bed_leveling::measure_point_with_encoder()
{
	...
	while (ubl_lcd_clicked()) {
		delay(50);    // wait for user to release encoder wheel
	}
	delay(50);  // debounce
...
void unified_bed_leveling::manually_probe_remaining_mesh(const float &lx, const float &ly, const float &z_clearance,
		const float &thick, const bool do_ubl_mesh_map)
{
	...
	do {
		...
		if (!position_is_reachable_raw_xy(rawx, rawy)) {
			break;    // SHOULD NOT OCCUR (find_closest_mesh_point only returns reachable points)
		}
		...
		if (do_ubl_mesh_map) {
			display_map(g29_map_type);    // show user where we're probing
		}

		serialprintPGM(parser.seen('B') ? PSTR(MSG_UBL_BC_INSERT) : PSTR(MSG_UBL_BC_INSERT2));

		const float z_step =
			0.01;                                        // existing behavior: 0.01mm per click, occasionally step
...
void unified_bed_leveling::g29_what_command()
{
	...
	SERIAL_PROTOCOLLNPAIR("EEPROM free for UBL: ",
						  hex_address((void *)(settings.get_end_of_meshes() - settings.get_start_of_meshes())));
	safe_delay(50);
...
mesh_index_pair unified_bed_leveling::find_closest_mesh_point_of_type(const MeshPointType type, const float &lx,
		const float &ly, const bool probe_as_reference, unsigned int bits[16], const bool far_flag)
{
					...
					for (uint8_t k = 0; k < GRID_MAX_POINTS_X; k++) {
						for (uint8_t l = 0; l < GRID_MAX_POINTS_Y; l++) {
							if (i != k && j != l && !isnan(z_values[k][l])) {
								...
								distance += HYPOT(MESH_X_DIST, MESH_Y_DIST) / log(HYPOT((i - k) * (MESH_X_DIST) + .001,
											(j - l) * (MESH_Y_DIST)) + .001);
							}
						}
					}
...
void unified_bed_leveling::fine_tune_mesh(const float &lx, const float &ly, const bool do_ubl_mesh_map)
{
	...
	do {
		...
		if (location.x_index < 0) {
			break;    // stop when we can't find any more reachable points.
		}
		...
		if (!position_is_reachable_raw_xy(rawx,
										  rawy)) { // SHOULD NOT OCCUR because find_closest_mesh_point_of_type will only return reachable
			break;
		}
		...
		if (do_ubl_mesh_map) {
			display_map(g29_map_type);    // show the user which point is being adjusted
		}
...
void unified_bed_leveling::tilt_mesh_based_on_probed_grid(const bool do_ubl_mesh_map)
{
	...
	for (uint8_t ix = 0; ix < g29_grid_size; ix++) {
		...
		for (int8_t iy = 0; iy < g29_grid_size; iy++) {
			...
			float measured_z = probe_pt(LOGICAL_X_POSITION(x), LOGICAL_Y_POSITION(y), parser.seen('E'),
										g29_verbose_level); // TODO: Needs error handling
			#if ENABLED(DEBUG_LEVELING_FEATURE)
			if (DEBUGGING(LEVELING)) {
				SERIAL_CHAR('(');
