...
void unified_bed_leveling::G26()
{
	...
	if (axis_unhomed_error() || parse_G26_parameters()) {
		return;
	}
	...
	if (g26_prime_flag && prime_nozzle()) {
		goto LEAVE;
	}
	...
	do {
		...
		if (location.x_index >= 0 && location.y_index >= 0) {
			...
			for (tmp = start_angle; tmp < end_angle - 0.1; tmp += 30.0) {
				...
				if (!position_is_reachable_raw_xy(x, y) || !position_is_reachable_raw_xy(xe, ye)) {
					continue;
				}
				...
				print_line_from_here_to_there(LOGICAL_X_POSITION(x), LOGICAL_Y_POSITION(y), g26_layer_height, LOGICAL_X_POSITION(xe),
											  LOGICAL_Y_POSITION(ye), g26_layer_height);
...
bool unified_bed_leveling::look_for_lines_to_connect()
{
	...
	for (uint8_t i = 0; i < GRID_MAX_POINTS_X; i++) {
		for (uint8_t j = 0; j < GRID_MAX_POINTS_Y; j++) {
			...
			#if ENABLED(NEWPANEL)
			if (user_canceled()) {
				return true;    // Check if the user wants to stop the Mesh Validation
			}
			#endif
				...
				if (is_bit_set(circle_flags, i, j) && is_bit_set(circle_flags, i + 1, j)) { // check if we can do a line to the left
					if (!is_bit_set(horizontal_mesh_line_flags, i, j)) {
						...
						if (position_is_reachable_raw_xy(sx, sy) && position_is_reachable_raw_xy(ex, ey)) {
							...
							print_line_from_here_to_there(LOGICAL_X_POSITION(sx), LOGICAL_Y_POSITION(sy), g26_layer_height, LOGICAL_X_POSITION(ex),
														  LOGICAL_Y_POSITION(ey), g26_layer_height);
						}
...
void unified_bed_leveling::move_to(const float &x, const float &y, const float &z, const float &e_delta)
{
	...
	bool has_xy_component = (x != current_position[X_AXIS] ||
							 y != current_position[Y_AXIS]); // Check if X or Y is involved in the movement.
	...
	if (g26_debug_flag) {
		SERIAL_ECHOLNPAIR("in move_to() feed_value for XY:", feed_value);
	}
...
*/
void unified_bed_leveling::print_line_from_here_to_there(const float &sx, const float &sy, const float &sz,
		const float &ex, const float &ey, const float &ez)
{
	...
	if (dist_start > 2.0) {
		...
		move_to(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS] + 0.500,
				0.0);  // Z bump to minimize scraping
