...
#define LETTER_BIT(N) PARAM_BIT(LETTER_OFF(N))
	...
	static bool seen(const char c)
	{
		...
		if (b) {
			value_ptr = param[ind] ? command_ptr + param[ind] : (char *)NULL;
		}
		return b;
	}
...
#define SEEN_TEST(L) !!strchr(command_args, L)
	...
	inline static float value_linear_units()
	{
		return value_float() * linear_unit_factor;
	}
	inline static float value_axis_units(const AxisEnum axis)
	{
		return value_float() * axis_unit_factor(axis);
	}
	inline static float value_per_axis_unit(const AxisEnum axis)
	{
		return value_float() / axis_unit_factor(axis);
	}
	...
	FORCE_INLINE static float value_celsius_diff()
	{
		return value_float();
	}
	...
	FORCE_INLINE static bool     boolval(const char c, const bool dval = false)
	{
		return seen(c)    ? value_bool()         : dval;
	}
	...
	FORCE_INLINE static float    linearval(const char c, const float dval = 0.0)
	{
		return seenval(c) ? value_linear_units() : dval;
	}
	FORCE_INLINE static float    celsiusval(const char c, const float dval = 0.0)
	{
