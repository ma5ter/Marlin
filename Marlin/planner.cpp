...
void Planner::reverse_pass()
{
	...
	if (movesplanned() > 3) {
		...
		while (b != tail) {
			if (block[0] && TEST(block[0]->flag, BLOCK_BIT_START_FROM_FULL_HALT)) {
				break;
			}
...
void Planner::getHighESpeed()
{
	...
	if (t < oldt) {
		t = t * (1 - (AUTOTEMP_OLDWEIGHT)) + oldt * (AUTOTEMP_OLDWEIGHT);
	}
	oldt = t;
...
void Planner::apply_leveling(float &lx, float &ly, float &lz)
{
	...
	if (planner.z_fade_height && planner.z_fade_height <= RAW_Z_POSITION(lz)) {
		return;
	}
...
void Planner::unapply_leveling(float logical[XYZ])
{
	...
	#if ENABLED(ENABLE_LEVELING_FADE_HEIGHT)
	if (z_fade_height && RAW_Z_POSITION(logical[Z_AXIS]) >= z_fade_height) {
		return;
	}
	#endif
...
*/
void Planner::_buffer_line(const float &a, const float &b, const float &c, const float &e, float fr_mm_s,
						   const uint8_t extruder)
{
	...
	#if CORE_IS_XY
	if (da < 0) {
		SBI(dm, X_HEAD);    // Save the real Extruder (head) direction in X Axis
	}
	if (db < 0) {
		SBI(dm, Y_HEAD);    // ...and Y
	}
	...
	if (db < 0) {
		SBI(dm, Y_HEAD);    // Save the real Extruder (head) direction in Y Axis
	}
	if (dc < 0) {
		SBI(dm, Z_HEAD);    // ...and Z
	}
...
#define DISABLE_IDLE_E(N) if (!g_uc_extruder_last_move[N]) disable_E##N();
	...
	*/
	...
	if (block->steps[X_AXIS] < MIN_STEPS_PER_SEGMENT && block->steps[Y_AXIS] < MIN_STEPS_PER_SEGMENT &&
	...
	#if ENABLED(SLOWDOWN) || ENABLED(LCDKIND_CHARACTER) || defined(XY_FREQUENCY_LIMIT)
	...
	if (WITHIN(moves_queued, 2, (BLOCK_BUFFER_SIZE) / 2 - 1)) {
		if (segment_time < min_segment_time) {
			...
			#if defined(XY_FREQUENCY_LIMIT) || ENABLED(LCDKIND_CHARACTER)
	...
	if (extruder == FILAMENT_SENSOR_EXTRUDER_NUM &&
		filwidth_delay_index[1] >= 0) {  //only for extruder with filament sensor and if ring buffer is initialized
	...
	LOOP_XYZE(i) {
		...
		#endif
		if (cs > max_feedrate_mm_s[i]) {
			NOMORE(speed_factor, max_feedrate_mm_s[i] / cs);
		}
	}
...
#define ACCEL_IDX 0
	...
	*/
	...
	if (moves_queued > 1 && previous_nominal_speed > 0.0001) {
		...
		float smaller_speed_factor = prev_speed_larger ? (block->nominal_speed / previous_nominal_speed) :
	...
	if (esteps && (block->steps[X_AXIS] || block->steps[Y_AXIS] || block->steps[Z_AXIS])) {
		...
		const float advance = ((STEPS_PER_CUBIC_MM_E) * (EXTRUDER_ADVANCE_K)) * HYPOT(current_speed[E_AXIS],
							  EXTRUSION_AREA) * 256;
...
void Planner::autotemp_M104_M109()
{
	...
	if (autotemp_enabled) {
		autotemp_factor = parser.value_celsius_diff();
	}
	if (parser.seen('S')) {
