...
LCD_CLASS lcd(LCD_I2C_ADDRESS, LCD_I2C_PIN_EN, LCD_I2C_PIN_RW, LCD_I2C_PIN_RS, LCD_I2C_PIN_D4, LCD_I2C_PIN_D5,
...
static void logo_lines(const char *const extra)
{
	...
	lcd_printPGM(PSTR("|Marlin|"));
	lcd_printPGM(extra);
	lcd.setCursor(indent, 2);
	lcd.write('\x02');
	lcd_printPGM(PSTR("------"));
	lcd.write('\x03');
}
...
static void lcd_implementation_status_screen()
{
	...
	if (card.isFileOpen() && (ELAPSED(millis(), progress_bar_ms + PROGRESS_BAR_MSG_TIME) || !lcd_status_message[0])) {
		...
		if (percent) {
			return lcd_draw_progress_bar(percent);
		}
	}
	...
	if (slen <= LCD_WIDTH) {
		...
		if (last_blink != blink) {
			...
			if (status_scroll_pos < slen) while (!PRINTABLE(lcd_status_message[status_scroll_pos])) {
					status_scroll_pos++;
				}
			if (++status_scroll_pos >= slen + 2) {
				status_scroll_pos = 0;
			}
		}
	}
...
static void lcd_implementation_drawmenu_static(const uint8_t row, const char *pstr, const bool center = true,
		const bool invert = false, const char *valstr = NULL)
{
	UNUSED(invert);
	char c;
	int8_t n = LCD_WIDTH;
...
static void lcd_implementation_drawmenu_generic(const bool sel, const uint8_t row, const char *pstr,
		const char pre_char, const char post_char)
{
	char c;
...
static void lcd_implementation_drawmenu_setting_edit_generic(const bool sel, const uint8_t row, const char *pstr,
		const char pre_char, const char *const data)
{
	char c;
...
static void lcd_implementation_drawmenu_setting_edit_generic_P(const bool sel, const uint8_t row, const char *pstr,
		const char pre_char, const char *const data)
{
	char c;
...
void lcd_implementation_drawedit(const char *pstr, const char *const value = NULL)
{
	...
	if (value != NULL) {
		lcd.write(':');
		const uint8_t valrow = (lcd_strlen_P(pstr) + 1 + lcd_strlen(value) + 1) > (LCD_WIDTH - 2) ? 2 :
							   1;  // Value on the next row if it won't fit
		lcd.setCursor((LCD_WIDTH - 1) - (lcd_strlen(value) + 1),
					  valrow);                                   // Right-justified, padded by spaces
...
static void lcd_implementation_drawmenu_sd(const bool sel, const uint8_t row, const char *const pstr,
...
static void lcd_implementation_drawmenu_sdfile(const bool sel, const uint8_t row, const char *pstr,
		const char *filename, char *const longFilename)
{
	lcd_implementation_drawmenu_sd(sel, row, pstr, filename, longFilename, 2, ' ');
}

static void lcd_implementation_drawmenu_sddirectory(const bool sel, const uint8_t row, const char *pstr,
		const char *filename, char *const longFilename)
{
...
void add_edges_to_custom_char(custom_char *const custom, coordinate *const ul, coordinate *const lr,
							  coordinate *const brc, const uint8_t cell_location);
...
coordinate pixel_location(int16_t x, int16_t y)
{
	...
	x++;
	y++; // +1 because lines on the left and top
...
coordinate pixel_location(uint8_t x, uint8_t y)
{
...
#define _ZLABEL(X,Y) _LABEL('Z',X,Y)
	...
	*/
	...
	x_map_pixels = (ULTRA_X_PIXELS_PER_CHAR) * (ULTRA_COLUMNS_FOR_MESH_MAP) -
				   2;  // minus 2 because we are drawing a box around the map
	y_map_pixels = (ULTRA_Y_PIXELS_PER_CHAR) * (ULTRA_ROWS_FOR_MESH_MAP) - 2;

	pixels_per_x_mesh_pnt = x_map_pixels / (GRID_MAX_POINTS_X);
	pixels_per_y_mesh_pnt = y_map_pixels / (GRID_MAX_POINTS_Y);

	if (pixels_per_x_mesh_pnt >=
		ULTRA_X_PIXELS_PER_CHAR) {         // There are only 2 custom characters available, so the X
		pixels_per_x_mesh_pnt =
			ULTRA_X_PIXELS_PER_CHAR;              // size of the mesh point needs to fit within them independent
		suppress_x_offset = 1;                                        // of where the starting pixel is located.
	}

	if (pixels_per_y_mesh_pnt >=
		ULTRA_Y_PIXELS_PER_CHAR) {         // There are only 2 custom characters available, so the Y
		pixels_per_y_mesh_pnt =
			ULTRA_Y_PIXELS_PER_CHAR;              // size of the mesh point needs to fit within them independent
	...
	bottom_line = pixels_per_y_mesh_pnt * (GRID_MAX_POINTS_Y) +
				  1;  // find location of bottome line within the character cell
...
void add_edges_to_custom_char(custom_char *const custom, coordinate *const ul, coordinate *const lr,
							  coordinate *const brc, uint8_t cell_location)
{
	uint8_t i, k;
	...
	*/
	if (ul->row == 0 && ((cell_location & TOP_LEFT) ||
						 (cell_location & TOP_RIGHT))) {   // Only fill in the top line for the top character cells
		...
		else if ((cell_location & TOP_LEFT) ||
				 lr->column != brc->column) { // Multiple column in the middle or with right cell in middle
	...
	*/
	if ((cell_location & TOP_LEFT) || (cell_location & LOWER_LEFT)) {
		if (ul->column == 0) {                                              // Left column of characters on LCD Display
			k = ul->row == brc->row ? brc->y_pixel_offset :
				ULTRA_Y_PIXELS_PER_CHAR; // If it isn't the last row... do the full character cell
			for (i = 0; i < k; i++) {
