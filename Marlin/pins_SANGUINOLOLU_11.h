...
#if DISABLED(SPINDLE_LASER_ENABLE) && ENABLED(SANGUINOLOLU_V_1_2) && !(ENABLED(LCDKIND_CHARACTER) && ENABLED(NEWPANEL))  // try to use IO Header
...
//
	...
	#if !MB(AZTEEG_X1) && ENABLED(SANGUINOLOLU_V_1_2) && !(ENABLED(LCDKIND_CHARACTER) && ENABLED(NEWPANEL))  // try to use IO Header
