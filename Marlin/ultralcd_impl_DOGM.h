...
static void lcd_setFont(const char font_nr)
{
	switch (font_nr) {
		...
		case FONT_MENU       : {
			...
			currentfont = FONT_MENU;
		};
		break;
		case FONT_SPECIAL    : {
			u8g.setFont(FONT_SPECIAL_NAME);
			currentfont = FONT_SPECIAL;
		};
		break;
		case FONT_MENU_EDIT  : {
			u8g.setFont(FONT_MENU_EDIT_NAME);
			currentfont = FONT_MENU_EDIT;
		};
		break;
		break;
	}
...
#define STATUS_BASELINE (55 + INFO_FONT_HEIGHT)
	...
	if (PAGE_CONTAINS(STATUS_BASELINE - (INFO_FONT_HEIGHT - 1), STATUS_BASELINE)) {
		...
		if (PENDING(millis(), previous_lcd_status_ms +
					5000UL)) {  //Display both Status message line and Filament display on the last line
...
static void lcd_implementation_hotend_status(const uint8_t row)
{
	...
	if (!PAGE_CONTAINS(row_y1 + 1, row_y2 + 2)) {
		return;
	}
...
static void _drawmenu_setting_edit_generic(const bool isSelected, const uint8_t row, const char *pstr,
		const char *const data, const bool pgm)
{
...
void lcd_implementation_drawedit(const char *const pstr, const char *const value = NULL)
{
	...
	if (labellen <= LCD_WIDTH_EDIT - 1) {
		if (labellen + vallen + 2 >= LCD_WIDTH_EDIT) {
			rows = 2;
		}
...
static void _drawmenu_sd(const bool isSelected, const uint8_t row, const char *const pstr, const char *filename,
						 char *const longFilename, const bool isDir)
{
	UNUSED(pstr);
